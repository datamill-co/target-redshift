...
| `max_batch_rows`            | `["integer", "null"]` | `200000`                         | The maximum number of rows to buffer in memory before writing to the destination table in Redshift
...
| `truncate_columns`      | `["boolean", "null"]` | `False`    | Whether the Target should truncate values longer than `default_column_length` when loading into Redshift. Adds the `TRUNCATECOLUMNS` data conversion parameter to Redshift's COPY command.
